            ...
            cliSelect({
                ...
                filterAsk(0, () => {
                    ...
                    if (folders.length > 0 && files.length > 0) {
                                        ...
                                        exec('rm -rf temp/ && wit EXTRACT ' + path.resolve(romRoot, wbfsFiles[0]) + ' temp/', (error, stdout, stderr) => {
                                            ...
                                            folders.forEach(folder => {
                                                if (fs.existsSync(path.join(sdRoot, folder.external))) {
                                                    ...
                                                    console.log(chalk.dim('Copied (path) "' + path.join(sdRoot, folder.external) + '" to "' + path.join(__dirname, '/temp/files', folder.disc) + '" - ' + stdout));
                                                } else {
                                            ...
                                            files.forEach(file => {
                                                if (fs.existsSync(path.join(sdRoot, file.external))) {
                                                    ...
                                                    console.log(chalk.dim('Copied (file) "' + path.join(sdRoot, file.external) + '" to "' + (adjustedPath || path.join(__dirname, '/temp/files', file.disc)) + '" - Destination was ' + (adjustedPath ? chalk.green('found') : chalk.blue('created')) + ' - ' + stdout));
                                                } else {
                        ...
                        console.error('No patches were selected. Ending script.');
                    }
